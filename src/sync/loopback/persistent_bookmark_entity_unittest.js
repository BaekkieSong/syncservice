...
const util = require(path.join(
  workspaceDir,
  "src/sync/loopback/server_entity_static_api.js"
));
...
function checkSetValidEntity(lsEntity, pbSyncEntity) {
  ...
  assert(lsEntity.lastModifiedTime >= pbSyncEntity.getMtime()); // Updated
  assert(
    lsEntity.specifics.getBookmark().getTitle() ==
      pbSyncEntity.getSpecifics().getBookmark().getTitle()
  );
  assert(
    lsEntity.specifics.getBookmark().getUrl() ==
      pbSyncEntity.getSpecifics().getBookmark().getUrl()
  );
  // 북마크에 없는 필드값은 Loopback Server Entity변환될 때 추가하지 않음
  assert(
    lsEntity.serverDefinedUniqueTag != pbSyncEntity.getServerDefinedUniqueTag()
  );
}
...
assert(
  pbNewLoopbackServerEntity.getType() ==
    proto.sync_pb.LoopbackServerEntity.Type.BOOKMARK
);
