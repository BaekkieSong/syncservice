...
/* static & private */
function createNewInternal(id, version, clientDefinedUniqueTag) { // string, int64_t, string
  const modelType = se.getModelTypeFromId(id);
  if (modelType == mt.ModelType.UNSPECIFIED) {
    console.error('\x1b[35m%s\x1b[0m', "Invalid ID was given:", id);
    return undefined;
  };
  return new PersistentTombstoneEntity(id, version, modelType, clientDefinedUniqueTag);
