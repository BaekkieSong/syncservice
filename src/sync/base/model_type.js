...
const ModelTypeInfoMap = [
  ...
  [ModelType.AUTOFILL_PROFILE, "AUTOFILL_PROFILE", "autofill_profiles",
    "Autofill Profiles",
  ...
  [ModelType.AUTOFILL_WALLET_DATA, "AUTOFILL_WALLET", "autofill_wallet",
    "Autofill Wallet",
  proto.sync_pb.EntitySpecifics.SpecificsVariantCase.AUTOFILL_WALLET, 34],
  [ModelType.AUTOFILL_WALLET_METADATA, "WALLET_METADATA",
  ...
  [ModelType.SEARCH_ENGINES, "SEARCH_ENGINE", "search_engines",
    "Search Engines",
  ...
  [ModelType.EXTENSION_SETTINGS, "EXTENSION_SETTING", "extension_settings",
    "Extension settings",
  proto.sync_pb.EntitySpecifics.SpecificsVariantCase.EXTENSION_SETTING, 14],
  [ModelType.APP_NOTIFICATIONS, "APP_NOTIFICATION", "app_notifications",
    "App Notifications",
  proto.sync_pb.EntitySpecifics.SpecificsVariantCase.APP_NOTIFICATION, 15],
  [ModelType.HISTORY_DELETE_DIRECTIVES, "HISTORY_DELETE_DIRECTIVE",
  ...
  [ModelType.SYNCED_NOTIFICATION_APP_INFO, "SYNCED_NOTIFICATION_APP_INFO",
  ...
  [ModelType.FAVICON_IMAGES, "FAVICON_IMAGE", "favicon_images",
    "Favicon Images",
  proto.sync_pb.EntitySpecifics.SpecificsVariantCase.FAVICON_IMAGE, 23],
  [ModelType.FAVICON_TRACKING, "FAVICON_TRACKING", "favicon_tracking",
    "Favicon Tracking",
  ...
  [ModelType.PRIORITY_PREFERENCES, "PRIORITY_PREFERENCE",
    "priority_preferences", "Priority Preferences",
  proto.sync_pb.EntitySpecifics.SpecificsVariantCase.PRIORITY_PREFERENCE, 21],
  [ModelType.SUPERVISED_USER_SETTINGS, "MANAGED_USER_SETTING",
    "managed_user_settings", "Managed User Settings",
  proto.sync_pb.EntitySpecifics.SpecificsVariantCase.MANAGED_USER_SETTING, 26],
  [ModelType.DEPRECATED_SUPERVISED_USERS, "MANAGED_USER", "managed_users",
    "Managed Users",
  ...
  [ModelType.DEPRECATED_ARTICLES, "ARTICLE", "deprecated_articles",
    "Deprecated Articles",
  ...
  [ModelType.DEPRECATED_WIFI_CREDENTIALS, "WIFI_CREDENTIAL",
    "wifi_credentials", "WiFi Credentials",
  proto.sync_pb.EntitySpecifics.SpecificsVariantCase.WIFI_CREDENTIAL, 32],
  [ModelType.SUPERVISED_USER_WHITELISTS, "MANAGED_USER_WHITELIST",
  ...
  [ModelType.MOUNTAIN_SHARES, "MOUNTAIN_SHARE", "mountain_shares",
    "Mountain Shares",
  ...
  [ModelType.SEND_TAB_TO_SELF, "SEND_TAB_TO_SELF", "send_tab_to_self",
    "Send Tab To Self",
...
for (let i in ModelTypeInfoMap) {
  kModelTypeInfoMap.set(
    ModelTypeInfoMap[i][0], new ModelTypeInfo(ModelTypeInfoMap[i]));
  if (i > 0 && SpecificsFieldName.hasOwnProperty(
    ...
    assert(kModelTypeInfoMap.get(
      ModelTypeInfoMap[i][0]).getSpecificsFieldName() ==
      SpecificsFieldName[getModelTypeNameFromModelType(
...
function getModelType(pbSyncEntity) {  // sync_pb::SyncEntity
  ...
  let specificsModelType =
    getModelTypeFromSpecifics(pbSyncEntity.getSpecifics());
  if (specificsModelType != ModelType.UNSPECIFIED) {
    return specificsModelType;
  }
  if (pbSyncEntity.getServerDefinedUniqueTag() !=
    undefined && isFolder(pbSyncEntity)) {
...
function getModelTypeFromSpecifics(pbEntitySpecifics) {
  assert(43 == ModelType.MODEL_TYPE_COUNT,
  ...
  if (pbEntitySpecifics.hasExtension$())  // TODO: Generate 버그. 확인 필요
...
const kUserSelectableDataTypeNames = [
  ...
  "typedUrls", "extensions", "apps", "readingList", "tabs"];

// 값이 하나만 있는 것을 보장하려면 new Set으로 감쌀 것...
const protocolTypes = new Set(Object.values(modelTypeSet).filter((it) => {
  if (it >= ModelType.BOOKMARKS && it <= ModelType.EXPERIMENTS)
    return it;
}));
const userTypes = new Set(Object.values(modelTypeSet).filter((it) => {
  if (it >= FIRST_USER_MODEL_TYPE && it <= LAST_USER_MODEL_TYPE)
    return it;
}));
const alwaysPreferredUserTypes =
  new Set([ModelType.DEVICE_INFO, ModelType.USER_CONSENTS]);
const userSelectableTypes =
  new Set([
    ModelType.BOOKMARKS, ModelType.PREFERENCES, ModelType.PASSWORDS,
    ModelType.AUTOFILL, ModelType.THEMES, ModelType.TYPED_URLS,
    ModelType.EXTENSIONS, ModelType.APPS, ModelType.READING_LIST,
    ModelType.PROXY_TABS]);
const priorityUserTypes =
  new Set([ModelType.DEVICE_INFO, ModelType.PRIORITY_PREFERENCES]);
const proxyTypes = new Set(Object.values(modelTypeSet).filter((it) => {
  if (it >= FIRST_PROXY_TYPE && it <= LAST_PROXY_TYPE)
    return it;
}));
const controlTypes = new Set(Object.values(modelTypeSet).filter((it) => {
...
const coreTypes = new Set([
  ModelType.NIGORI, ModelType.EXPERIMENTS,
  ModelType.SUPERVISED_USER_SETTINGS, ModelType.SYNCED_NOTIFICATIONS,
  ModelType.SYNCED_NOTIFICATION_APP_INFO,
  ModelType.SUPERVISED_USER_WHITELISTS]);
const priorityCoreTypes = new Set([
  ModelType.NIGORI, ModelType.EXPERIMENTS, ModelType.SUPERVISED_USER_SETTINGS]);
const commitOnlyTypes =
  new Set([ModelType.USER_EVENTS, ModelType.USER_CONSENTS]);
...
function getSpecificsFieldNumberFromModelType(modelType) {
  assert(protocolTypes.has(modelType),
...
function modelTypeToString(modelType) {
  if (modelType >= ModelType.UNSPECIFIED &&
...
function modelTypeToStableIdentifier(modelType) {
  assert(modelType >= ModelType.UNSPECIFIED &&
    modelType < ModelType.MODEL_TYPE_COUNT);
...
function modelTypeToRootTag(modelType) {
  ...
  console.error('\x1b[31m%s\x1b[0m',
    'NOT REACHED: No known extension for model type:', modelType);
  return "Invalid";
};
...
function isRealDataType(modelType) {
  return (modelType >= FIRST_REAL_MODEL_TYPE &&
    modelType < ModelType.MODEL_TYPE_COUNT);
};
...
exports.getModelTypeFromSpecificsFieldNumber =
  getModelTypeFromSpecificsFieldNumber;
exports.getSpecificsFieldNumberFromModelType =
...
exports.getRealModelTypeToNotificationType =
  getRealModelTypeToNotificationType;  // replaced
exports.getNotificationTypeToRealModelType =
  getNotificationTypeToRealModelType;  // replaced
