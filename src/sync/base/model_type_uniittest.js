...
assert(
  44 == Object.entries(mt.ModelType).length,
  `ModelType length is ${Object.entries(mt.ModelType).length}`
);

assert(
  43 == mt.ModelType.MODEL_TYPE_COUNT,
...
assert(
  mt.getSpecificsFieldNumberFromModelType(
    mt.getModelTypeFromSpecificsFieldNumber(32904)
  ) == 32904
);

assert(
  mt.modelTypeToHistogramSuffix(mt.ModelType.BOOKMARKS) ==
    mt.getModelTypeInfoMap().get(mt.ModelType.BOOKMARKS).notificationType
);
assert(
  mt.modelTypeToStableIdentifier(mt.ModelType.BOOKMARKS) ==
    mt.getModelTypeInfoMap().get(mt.ModelType.BOOKMARKS).modelTypeHistogramVal +
      1
);

assert(
  mt.modelTypeFromString(
...
assert(
  mt.modelTypeToString(mt.ModelType.BOOKMARKS) ==
    mt.getModelTypeInfoMap().get(mt.ModelType.BOOKMARKS).modelTypeString
);
...
assert(
  "bookmarks" == mt.getModelTypeRootTag(mt.ModelType.BOOKMARKS),
  "BOOKMARKS's RootTag is 'bookmarks'"
);
assert(
  "themes" == mt.getModelTypeRootTag(mt.ModelType.THEMES),
  "THEMES's RootTag is 'themes'"
);

assert(
  "BOOKMARK" == mt.getRealModelTypeToNotificationType(mt.ModelType.BOOKMARKS),
  "Check: this type is real model type"
);
assert(
  "" == mt.getRealModelTypeToNotificationType(43),
  "Check: this model type is not real model type"
);
assert(
  mt.ModelType.BOOKMARKS == mt.getNotificationTypeToRealModelType("BOOKMARK"),
  "Check: this notification type is exists"
);
assert(
  mt.ModelType.UNSPECIFIED ==
...
assert(
  mt.isTypeWithServerGeneratedRoot(mt.ModelType.BOOKMARKS),
  "bookmarks is generated server"
);
assert(
  mt.isTypeWithServerGeneratedRoot(mt.ModelType.NIGORI),
  "nigori is generated server"
);
assert(
  mt.isTypeWithClientGeneratedRoot(mt.ModelType.THEMES),
