...
/* 서버 초기화 */
function initSetting(request) {
  assert(request);
  console.log("Initialize settings");
}

function isFaviconURL(request) {
...
app.post("/chrome-sync/dev/command/", (request, response) => {
  assert(request);
  response.end("wow");
});
...
app.post("/", (request, response) => {
  ...
  request.on("end", () => {
    ...
    if (request.get("content-encoding") == "gzip") {
      ...
      pbRequest = proto.sync_pb.ClientToServerMessage.deserializeBinary(
        pbRequest
      );
...
function handle(pbRequest, pbResponse, parsedData) {
  ...
  if (
    ...
    for (let index in pbRequest.getGetUpdates().getFromProgressMarkerList()) {
      if (
        mt.getModelTypeFromSpecificsFieldNumber(
          pbRequest
            .getGetUpdates()
        ...
        walletMarker = pbRequest.getGetUpdates().getFromProgressMarkerList()[
          index
        ];
...
function makeResponseBody(pbResponse) {
  ...
  pbResponse.setStoreBirthday(sync.loopbackServer.getStoreBirthday());
